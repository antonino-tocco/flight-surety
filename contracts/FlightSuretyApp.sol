...
contract FlightSuretyApp {
    ...
    uint256 private constant MINIMUM_INSURANCE_AMOUNT = 1 ether;
    ...
    *
    */
    constructor(address dataContract, address airlineAddress) public
    {
        contractOwner = msg.sender;
        flightSuretyData = FlightSuretyData(payable(dataContract));
        //authorize app contract to data contract for register airline
        flightSuretyData.authorizeAppContract(address(this));
        //register first airline
        flightSuretyData.registerAirline("First airline", airlineAddress);
    }
    ...
    *
    */
    function registerAirline(string memory name, address airlineAddress) external returns (bool success, uint256 votes)
    {
        ...
        require(!flightSuretyData.isAirlineRegistered(airlineAddress) == false, "Airline is already registered");
        require(!flightSuretyData.isAirlinePending(airlineAddress) == false, "Airline is already pending registration");
        if (flightSuretyData.getRegisteredAirlineCount() < AIRLINES_THRESHOLD) {
            flightSuretyData.registerAirline(name, airlineAddress);
            return (true, 0);
        } else {
            flightSuretyData.addPendingAirline(name, airlineAddress, msg.sender);
            return (true, flightSuretyData.getVotesForAirline(airlineAddress));
        }
    }
